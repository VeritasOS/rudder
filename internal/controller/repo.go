...
func (rc *RepoController) ChartDetails(repoName, chartName, chartVersion string) (chartDetail *ChartDetail, err error) {
	...
	log.Infof("fetching Helm Chart '%s' from URL: %s", chartName, chartURL)
...
func (rc *RepoController) RefreshCache(repoName string) ([]byte, error) {
	...
	if err != nil {
		log.Debugf("unable to refresh cache for repo %s", repoName)
		return nil, err
	}
	log.Info("Refresh cache is successful for repo ", repoName)
	return data, nil
}
